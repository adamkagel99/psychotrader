...
function TodayStrip(props){
  ...
  var tiles=[
    ...
    {label:"Session",value:(function(){if(props.phase==="closed")return getPhaseLabel(props.phase);var s=getSessionByID(props.phase);return s?(s.name||s.label||props.phase):"Unknown Session";})(),color:props.phase!=="closed"?"#a5b4fc":"#94a3b8",small:true},
...
// CHANGED: Reusable live (open) trades panel — shown on both Journal and Home.
function LiveTradesPanel(props){
  var liveTrades=props.liveTrades||[];
  ...
  return (
    <div style={CS({marginBottom:16,border:"1px solid #ea580c",background:"#1c1108"})}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
        ...
        <span style={{fontSize:11,color:"#fdba74",fontWeight:600,background:"#7c2d1244",padding:"1px 7px",borderRadius:3}}>{liveTrades.length} open</span>
      </div>
      {liveTrades.map(function(lt){
...
function DashboardTab(props){
  ...
  return (
    <div style={{paddingTop:16}}>
      ...
      {/* CHANGED: live (open) trades surfaced on Home too. */}
      {props.phase!=="closed"&&<LiveTradesPanel liveTrades={props.liveTrades} openLiveTrade={props.openLiveTrade}/>}
...
function TradesTab(props){
  ...
  useEffect(function(){
    try{
      ...
      // Don't clobber an entry we already have in memory if storage read came back empty
      // (e.g. localStorage blocked in a sandbox). Only overwrite when storage actually has data.
      setTodayJournalEntry(function(prev){return entry||prev||null;});
    }catch(e){/* keep whatever is already in state */}
  },[props.reloadKey]);
  ...
  return (
    <div style={{paddingTop:16}}>
      ...
      {phase!=="closed"&&isToday&&<LiveTradesPanel liveTrades={props.liveTrades} openLiveTrade={props.openLiveTrade}/>}
      ...
      {isToday&&!todayJournalEntry&&(state.trades||[]).length>0&&(
        <div style={CS({marginTop:18,border:"1px solid #4338ca44"})}>
          ...
          <button onClick={function(){
            ...
            try{
              localStorage.setItem(key,JSON.stringify(entry));
            }catch(e){/* storage may be blocked (e.g. sandbox) — entry still held in React state below */}
            // Hold the saved entry in memory and switch to the saved-day view. We intentionally do
            // NOT bumpReloadKey here: the reload effect re-reads localStorage, and if the write was
            // blocked it would read null and wipe this entry, making the save appear to do nothing.
            setTodayJournalEntry(entry);
...
function App(){
  ...
  // CHANGED: manual view-mode toggle (laptop multi-column vs mobile single-column shell).
  var mobile=settings.viewMode==="mobile";
  function toggleViewMode(){setSettings(function(s){return Object.assign({},s,{viewMode:s.viewMode==="mobile"?"laptop":"mobile"});});}
  return (
